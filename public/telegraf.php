...
<main class="admin-page">
  ...
  <p class="hint">Each edit workstation runs Telegraf and posts metrics here. After the first sample the bay appears on <a href="index.php">Live</a> — Jump is not required. Do this once per machine (as a local admin).</p>

  <section class="admin-section">
    <h2>1. Ingest token</h2>
    <p class="hint" id="tokenHint">An admin generates the Bearer token in Settings → Telegraf ingest. The same token is used on every bay. The token is shown only once at generate time.</p>
  ...
  <section class="admin-section">
    <h2>2. Install Telegraf (Windows)</h2>
    <ol class="install-steps">
      <li>Download the Windows amd64 MSI from the <a href="https://github.com/influxdata/telegraf/releases/latest" target="_blank" rel="noopener">Telegraf releases</a> page, or run <code>winget install InfluxData.Telegraf</code>.</li>
      <li>Install the MSI. The service name is <code>telegraf</code>.</li>
      <li>NVIDIA GPUs: install the NVIDIA driver so <code>nvidia-smi</code> works. Skip the GPU input if the bay has no NVIDIA card.</li>
    </ol>
  </section>

  <section class="admin-section">
    <h2>3. telegraf.conf</h2>
    <p class="hint">Replace <code>YOUR_INGEST_TOKEN</code> with the token from Settings. Save as <code>C:\Program Files\InfluxData\telegraf\telegraf.conf</code> (MSI) or <code>C:\Program Files\Telegraf\telegraf.conf</code> (winget). Then restart the service.</p>
  ...
  <section class="admin-section">
    ...
    <p class="hint">The copied config includes <code>inputs.win_eventlog</code> for Event IDs 13, 41, 1074, 6006, 6008, 1001 (System/Application) and 19 / 20 / 43 (Windows Update). The Telegraf service must run as <strong>Local System</strong> (the MSI default) so it can read the System log. Events show on <a href="windows.php">Windows</a> and are kept for 90 days. This is log metadata (who rebooted, bugcheck code) — not <code>MEMORY.DMP</code> files. Collection starts from install time (<code>from_beginning = false</code>).</p>
  </section>
  ...
  <section class="admin-section">
    ...
    <pre class="copy-block" id="telStart">Restart-Service telegraf
Get-Content "$env:ProgramFiles\InfluxData\telegraf\telegraf.conf" -ErrorAction SilentlyContinue | Select-Object -First 1
Get-WinEvent -LogName Application -ProviderName telegraf -MaxEvents 8 -ErrorAction SilentlyContinue</pre>
    <ol class="install-steps">
